...
export default function CustomersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const result = await api.customers(`&page=${page}${query ? `&search=${encodeURIComponent(query)}` : ""}`); setCustomers(result.data); setTotalPages(result.meta.totalPages); }
  ...
  function openEdit(customer: Customer) {
    ...
    setForm({ name: customer.name, document: customer.document ?? "", email: customer.email ?? "", phone: customer.phone ?? "", cep: customer.cep ?? "", city: customer.city ?? "", state: customer.state ?? "" });
    setError("");
    setDialog(true);
  }
